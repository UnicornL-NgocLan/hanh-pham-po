...
export const exportPurchaseOrderToExcel = async (po, data) => {
    ...
    worksheet.getCell('G6').value = `Ngày đặt hàng: ${moment(po.date_ordered)
        .add(7, 'hour')
        .format('DD/MM/YYYY')}`
    // Apply font style
    ...
    worksheet.getCell('A9').value = {
        richText: [
            ...
            {
                text: po?.buyer_id?.name.toUpperCase(),
    ...
    worksheet.getCell('H9').value = {
        richText: [
            ...
            {
                text: po?.partner_id?.name.toUpperCase(),
                font: { name: 'Times New Roman', size: 14 },
            },
        ],
    }
    ...
    worksheet.getCell(
        'A10'
    ).value = `Địa chỉ: ${po?.buyer_id?.address}, ${po?.buyer_id?.district}`
    // Apply font style
    worksheet.getCell('A10').font = {
        name: 'Times New Roman',
        size: 16,
    ...
    worksheet.getCell('H10').value = `Địa chỉ: ${po?.partner_id?.address},`
    // Apply font style
    worksheet.getCell('H10').font = {
        name: 'Times New Roman',
        size: 16,
    ...
    worksheet.getCell(
        'A11'
    ).value = `${po?.buyer_id?.city}, ${po?.buyer_id?.country}`
    // Apply font style
    ...
    worksheet.getCell(
        'H11'
    ).value = `${po?.buyer_id?.district}, ${po?.buyer_id?.city}, ${po?.buyer_id?.country}`
    // Apply font style
    ...
    worksheet.getCell('A12').value = `Mã số thuế: ${po?.buyer_id?.vat}`
    // Apply font style
    ...
    worksheet.getCell('H12').value = `Mã số thuế: ${po?.partner_id?.vat}`
    // Apply font style
    ...
    worksheet.getCell('A13').value = `Điện thoại: ${po?.buyer_id?.phone}`
    // Apply font style
    ...
    worksheet.getCell('H13').value = `Điện thoại: ${po?.partner_id?.phone}`
    // Apply font style
    ...
    worksheet.getCell('A15').value = `Tài khoản: ${po?.buyer_id?.accountNumber}`
    // Apply font style
    ...
    worksheet.getCell(
        'H15'
    ).value = `Tài khoản: ${po?.partner_id?.accountNumber}`
    // Apply font style
        ...
        for (let j = 0; j < listOfColumn.length; j++) {
            ...
            switch (listOfColumn[j]) {
                ...
                case 'J':
                    ...
                    break
                case 'K':
                    worksheet.getCell(`${listOfColumn[j]}${rowNum}`).numFmt =
                        '#,##0'
                ...
                case 'L':
                    worksheet.getCell(`${listOfColumn[j]}${rowNum}`).numFmt =
                        '#,##0'
                    cellValue = data[i].price_unit
                    break
                case 'M':
                    worksheet.getCell(`${listOfColumn[j]}${rowNum}`).numFmt =
                        '#,##0'
                    cellValue = data[i].quantity * data[i].price_unit
                    break
                default:
                    cellValue = ''
            }
    ...
    worksheet.getCell(`C${endNumRow + 4}`).value = po.date_deliveried
        ? `${moment(po.date_deliveried).add(7, 'hour').format('DD/MM/YYYY')}`
        : 'thông báo sau'
    // Apply font style
